...
function solvers_dict(
  ...
  out_dict = Dict(:ripqp => ripqp1_stats)
...
function factorizations_dict(
  ...
  run_ma57 && (ripqp_ma57_stats = open_file(string("ripqp_ma57_", type)))
  run_ma97 && (ripqp_ma97_stats = open_file(string("ripqp_ma971_", type)))
...
function multi_ma57_dict(data_path::String; type::String = "lp")
  open_file(fname; data_path = data_path) = CSV.read(joinpath(data_path, string(fname, ".csv")), DataFrame)
  ripqp_ma57_stats = open_file(string("ripqp_ma57_", type))
...
function save_all_profiles(
  ...
  netlib_profile_time(solver_stats_lp, joinpath(profile_path, string("solvers_time_lp", plot_extension))) # fig 2
  ...
  mm_profile_time(solver_stats_qp, joinpath(profile_path, string("solvers_time_qp", plot_extension))) # fig 3
  ...
  netlib_profile_time(factorization_stats_lp, joinpath(profile_path, string("factorizations_time_lp", plot_extension))) # fig 4
  ...
  mm_profile_time(factorization_stats_qp, joinpath(profile_path, string("factorizations_time_qp", plot_extension))) # fig 5

  stats_cc_lp = centrality_corr_dict(data_path; type = "lp")
  netlib_profile_time(stats_cc_lp, joinpath(profile_path, string("cc_time_lp", plot_extension))) # fig 6
  stats_cc_qp = centrality_corr_dict(data_path; type = "qp")
  mm_profile_time(stats_cc_qp, joinpath(profile_path, string("cc_time_qp", plot_extension))) # fig 7

  stats_multi_lp = multi_dict(data_path; type = "lp")
  netlib_profile_energy(stats_multi_lp, joinpath(profile_path, string("multi_energy_lp", plot_extension))) # fig 8
  stats_multi_qp = multi_dict(data_path; type = "qp")
  mm_profile_energy(stats_multi_qp, joinpath(profile_path, string("multi_energy_qp", plot_extension))) # fig 9

  if run_ma57
    stats_ma57_mutli1_lp = multi_ma57_dict(data_path; type = "lp")
    netlib_profile_time(stats_ma57_mutli1_lp, joinpath(profile_path, string("ma57_multi1_time_lp", plot_extension))) # fig 10
    stats_ma57_mutli1_qp = multi_ma57_dict(data_path; type = "qp")
    mm_profile_time(stats_ma57_mutli1_qp, joinpath(profile_path, string("ma57_multi1_time_qp", plot_extension))) # fig 11

    stats_ma57_mutli2_lp = multi2_ma57_dict(data_path; type = "lp")
    netlib_profile_time(stats_ma57_mutli2_lp, joinpath(profile_path, string("ma57_multi2_time_lp", plot_extension))) # fig 12
    stats_ma57_mutli2_qp = multi2_ma57_dict(data_path; type = "qp")
    mm_profile_time(stats_ma57_mutli2_qp, joinpath(profile_path, string("ma57_multi2_time_qp", plot_extension))) # fig 13

    stats_ma57_mutli_all_lp = multi_ma57_all_dict(data_path; type = "lp")
    netlib_profile_energy(stats_ma57_mutli_all_lp, joinpath(profile_path, string("ma57_multi_all_energy_lp", plot_extension))) # fig 14
    stats_ma57_mutli_all_qp = multi_ma57_all_dict(data_path; type = "qp")
    mm_profile_energy(stats_ma57_mutli_all_qp, joinpath(profile_path, string("ma57_multi_all_energy_qp", plot_extension))) # fig 15
  end

  stats_mutlifact_lp = multifact_dict(data_path; type = "lp")
  netlib_profile_energy2(stats_mutlifact_lp, joinpath(profile_path, string("multifact_energy_lp", plot_extension))) # fig 16
  stats_mutlifact_qp = multifact_dict(data_path; type = "qp")
  mm_profile_energy2(stats_mutlifact_qp, joinpath(profile_path, string("multifact_energy_qp", plot_extension))) # fig 17

  stats_mutlifact_lim_lp = multifact_limited_dict(data_path; type = "lp")
  netlib_profile_energy2(stats_mutlifact_lim_lp, joinpath(profile_path, string("multifact_limited_energy_lp", plot_extension))) # fig 18
  stats_mutlifact_lim_qp = multifact_limited_dict(data_path; type = "qp")
  mm_profile_energy2(stats_mutlifact_lim_qp, joinpath(profile_path, string("multifact_limited_energy_qp", plot_extension))) # fig 19

  netlib_profile_time(stats_mutlifact_lim_lp, joinpath(profile_path, string("multifact_limited_time_lp", plot_extension))) # fig 20
  netlib_profile_time(stats_mutlifact_lim_qp, joinpath(profile_path, string("multifact_limited_time_qp", plot_extension))) # fig 21
end
